// Import secondary scenarios featuring other characters
import secondaryScenarios from './secondaryScenarios';

export const scenarios = [
  // Scenario: coffee_festival_v2 (choices swapped, impacts doubled)
  {
    id: "coffee_festival_v2",
    characterId: "bella",
    title: "Host a Coffee Festival",
    text: "Mr. Çelebi, a coffee festival could create buzz but is expensive. Should we host a big event, or a small tasting?",
    choiceA: {
      text: "Host a small tasting.",
      money: -6,    // originally -3, now -6
      popularity: 6, // originally 3, now 6
      operations: -2, // originally -1, now -2
      sustainability: 2, // originally 1, now 2
      experience: 2
    },
    choiceB: {
      text: "Host a big festival.",
      money: -40,  // originally -20, now -40
      popularity: 30, // originally 15, now 30
      operations: -14, // originally -7, now -14
      sustainability: 6, // originally 3, now 6
      experience: 5
    }
  },

  // Scenario: workplace_culture (choices not swapped, impacts doubled)
  {
    id: "workplace_culture",
    characterId: "bella",
    title: "Improve Workplace Culture",
    text: "Mr. Celebi, team-building activities could boost morale but cost money. Should we add them, or keep things as they are?",
    choiceA: {
      text: "Add team-building activities.",
      money: -20,  // originally -10, now -20
      popularity: 20, // originally 10, now 20
      operations: 0,
      sustainability: 0,
      experience: 3
    },
    choiceB: {
      text: "Keep things as they are.",
      money: 10,   // originally 5, now 10
      popularity: -10, // originally -5, now -10
      operations: 0,
      sustainability: 0,
      experience: 2
    }
  },

  // Scenario: new_technology_v2 (choices swapped, impacts doubled)
  {
    id: "new_technology_v2",
    characterId: "tom",
    title: "Invest in New Technology",
    text: "Mr. Çelebi, upgrading our coffee machines could improve quality and stocks but is expensive. Should we upgrade, or stick with old equipment?",
    choiceA: {
      text: "Stick with old equipment.",
      money: 14,  // originally 7, now 14
      popularity: -6, // originally -3, now -6
      operations: -6, // originally -3, now -6
      sustainability: 0,
      experience: 2
    },
    choiceB: {
      text: "Upgrade the machines.",
      money: -40, // originally -20, now -40
      popularity: 14, // originally 7, now 14
      operations: 20, // originally 10, now 20
      sustainability: 0,
      experience: 5
    }
  },

  // Scenario: vegan_options (choices not swapped, impacts doubled)
  {
    id: "vegan_options",
    characterId: "bella",
    title: "Offer Vegan and Gluten-Free Options",
    text: "Mr. Celebi, customers are asking for vegan and gluten-free options. Expanding the menu could attract new fans but strain our stocks. Should we do it, or keep the menu simple?",
    choiceA: {
      text: "Expand the menu.",
      money: -20,  // originally -10, now -20
      popularity: 20, // originally 10, now 20
      operations: -10, // originally -5, now -10
      sustainability: 10, // originally 5, now 10
      experience: 3
    },
    choiceB: {
      text: "Keep the menu simple.",
      money: 10,   // originally 5, now 10
      popularity: -10, // originally -5, now -10
      operations: 10,  // originally 5, now 10
      sustainability: 0,
      experience: 2
    }
  },

  // Scenario: sustainability_certification_v2 (choices swapped, impacts doubled)
  {
    id: "sustainability_certification_v2",
    characterId: "greta",
    title: "Participate in a Sustainability Certification",
    text: "Mr. Çelebi, joining a sustainability certification could boost our image but is costly. Should we participate, or opt out?",
    choiceA: {
      text: "Opt out.",
      money: 14,  // originally 7, now 14
      popularity: -6, // originally -3, now -6
      operations: 0,
      sustainability: -14, // originally -7, now -14
      experience: 2
    },
    choiceB: {
      text: "Join the certification.",
      money: -20, // originally -10, now -20
      popularity: 6,  // originally 3, now 6
      operations: 0,
      sustainability: 20, // originally 10, now 20
      experience: 3
    }
  },

  // Scenario: store_interiors (choices not swapped, impacts doubled)
  {
    id: "store_interiors",
    characterId: "max",
    title: "Upgrade Store Interiors",
    text: "Mr. Celebi, modern interiors could attract customers but are costly. Should we renovate, or make minor updates?",
    choiceA: {
      text: "Renovate with modern designs.",
      money: -60, // originally -30, now -60
      popularity: 30, // originally 15, now 30
      operations: 0,
      sustainability: 0,
      experience: 5
    },
    choiceB: {
      text: "Make minor updates.",
      money: -20, // originally -10, now -20
      popularity: 10,  // originally 5, now 10
      operations: 0,
      sustainability: 0,
      experience: 2
    }
  },

  // Scenario: carbon_footprint (choices swapped, impacts doubled)
  {
    id: "carbon_footprint",
    characterId: "greta",
    title: "Reduce Carbon Footprint",
    text: "Mr. Celebi, investing in renewable energy could greatly reduce our carbon footprint, but it's expensive. Should we invest now, or delay?",
    choiceA: {
      text: "Delay sustainability efforts.",
      money: 20,  // originally 10, now 20
      popularity: -10, // originally -5, now -10
      operations: 0,
      sustainability: -20, // originally -10, now -20
      experience: 2
    },
    choiceB: {
      text: "Invest in renewable energy.",
      money: -40, // originally -20, now -40
      popularity: 10,  // originally 5, now 10
      operations: 0,
      sustainability: 40, // originally 20, now 40
      experience: 5
    }
  },

  // Scenario: exotic_blend (choices not swapped, impacts doubled)
  {
    id: "exotic_blend",
    characterId: "max",
    title: "Develop a New Exotic Coffee Blend",
    text: "Mr. Celebi, should we invest heavily in R&D for a unique coffee blend? Customers might love the innovation, but it's costly and less sustainable. Or should we stick with cheaper, eco-friendly but traditional blends?",
    choiceA: {
      text: "Invest in the unique blend.",
      money: -40, // originally -20, now -40
      popularity: 30, // originally 15, now 30
      operations: 20, // originally 10, now 20
      sustainability: -10, // originally -5, now -10
      experience: 3
    },
    choiceB: {
      text: "Stick with traditional blends.",
      money: 20,  // originally 10, now 20
      popularity: -10, // originally -5, now -10
      operations: -10, // originally -5, now -10
      sustainability: 10,  // originally 5, now 10
      experience: 2
    }
  },

  // Scenario: employee_wages_v2 (choices swapped, impacts doubled)
  {
    id: "employee_wages_v2",
    characterId: "bella",
    title: "Increase Employee Wages",
    text: "Mr. Çelebi, our staff is asking for better wages. Should we give a big raise, or offer a small bonus to save money?",
    choiceA: {
      text: "Offer a small bonus.",
      money: -6,   // originally -3, now -6
      popularity: 6,  // originally 3, now 6
      operations: 0,
      sustainability: 0,
      experience: 2
    },
    choiceB: {
      text: "Give a big raise.",
      money: -30, // originally -15, now -30
      popularity: 20, // originally 10, now 20
      operations: 0,
      sustainability: 0,
      experience: 3
    }
  },

  // Scenario: bean_quality (choices not swapped, impacts doubled)
  {
    id: "bean_quality",
    characterId: "tom",
    title: "Improve Coffee Bean Quality",
    text: "Mr. Celebi, I found a supplier offering premium beans. The quality is great, but they're expensive and not eco-friendly. Should we switch, or stay with our current sustainable suppliers?",
    choiceA: {
      text: "Switch to the new supplier.",
      money: -30, // originally -15, now -30
      popularity: 20, // originally 10, now 20
      operations: 30, // originally 15, now 30
      sustainability: -20, // originally -10, now -20
      experience: 3
    },
    choiceB: {
      text: "Stay with current suppliers.",
      money: 10,   // originally 5, now 10
      popularity: -10, // originally -5, now -10
      operations: -10, // originally -5, now -10
      sustainability: 10,  // originally 5, now 10
      experience: 2
    }
  },

  // Scenario: ad_campaign (choices swapped, impacts doubled)
  {
    id: "ad_campaign",
    characterId: "max",
    title: "Launch a Major Advertising Campaign",
    text: "Mr. Celebi, a national advertising campaign could skyrocket our popularity, but it's expensive. Should we launch it, or focus on local marketing for now?",
    choiceA: {
      text: "Focus on local marketing.",
      money: -10,  // originally -5, now -10
      popularity: 10,  // originally 5, now 10
      operations: 0,
      sustainability: 0,
      experience: 2
    },
    choiceB: {
      text: "Launch the national campaign.",
      money: -50, // originally -25, now -50
      popularity: 40, // originally 20, now 40
      operations: 0,
      sustainability: 0,
      experience: 5
    }
  },

  // Scenario: loyalty_program (choices not swapped, impacts doubled)
  {
    id: "loyalty_program",
    characterId: "bella",
    title: "Implement a Customer Loyalty Program",
    text: "Mr. Celebi, a loyalty program could keep customers coming back. Should we offer generous rewards, or keep it minimal to save costs?",
    choiceA: {
      text: "Offer generous rewards.",
      money: -30, // originally -15, now -30
      popularity: 30, // originally 15, now 30
      operations: 0,
      sustainability: 0,
      experience: 3
    },
    choiceB: {
      text: "Offer minimal rewards.",
      money: -10,  // originally -5, now -10
      popularity: 10,  // originally 5, now 10
      operations: 0,
      sustainability: 0,
      experience: 2
    }
  },

  // Scenario: community_events (choices swapped, impacts doubled)
  {
    id: "community_events",
    characterId: "greta",
    title: "Host Community Events",
    text: "Mr. Celebi, hosting community events could boost our image. Should we go big with large-scale events, or keep it small and manageable?",
    choiceA: {
      text: "Host small gatherings.",
      money: -10,  // originally -5, now -10
      popularity: 10,  // originally 5, now 10
      operations: 0,
      sustainability: 4,  // originally 2, now 4
      experience: 2
    },
    choiceB: {
      text: "Organize large-scale events.",
      money: -40, // originally -20, now -40
      popularity: 30, // originally 15, now 30
      operations: -10, // originally -5, now -10
      sustainability: 10,  // originally 5, now 10
      experience: 5
    }
  },

  // Scenario: new_store (choices not swapped, impacts doubled)
  {
    id: "new_store",
    characterId: "max",
    title: "Open a New Store",
    text: "Mr. Celebi, we can open a new store in a prime location, but it's costly. Or should we choose a cheaper spot with less traffic?",
    choiceA: {
      text: "Open in a prime location.",
      money: -60, // originally -30, now -60
      popularity: 20, // originally 10, now 20
      operations: -20, // originally -10, now -20
      sustainability: 0,
      experience: 5
    },
    choiceB: {
      text: "Choose a cheaper location.",
      money: -20, // originally -10, now -20
      popularity: 10,  // originally 5, now 10
      operations: -10, // originally -5, now -10
      sustainability: 0,
      experience: 2
    }
  },

  // Scenario: supply_chain (choices swapped, impacts doubled)
  {
    id: "supply_chain",
    characterId: "tom",
    title: "Optimize Supply Chain Management",
    text: "Mr. Celebi, advanced logistics software could improve our supply chain, but it's expensive. Should we invest, or stick with our current systems?",
    choiceA: {
      text: "Stick with current systems.",
      money: 10,   // originally 5, now 10
      popularity: 0,
      operations: -10, // originally -5, now -10
      sustainability: 0,
      experience: 2
    },
    choiceB: {
      text: "Purchase the software.",
      money: -40, // originally -20, now -40
      popularity: 0,
      operations: 30, // originally 15, now 30
      sustainability: 0,
      experience: 3
    }
  },

  // Scenario: automation (choices not swapped, impacts doubled)
  {
    id: "automation",
    characterId: "tom",
    title: "Automate Coffee Production",
    text: "Mr. Celebi, automation could boost production but cut jobs and harm sustainability. Should we automate, or keep production manual?",
    choiceA: {
      text: "Invest in automation.",
      money: -50, // originally -25, now -50
      popularity: -20, // originally -10, now -20
      operations: 40, // originally 20, now 40
      sustainability: -10, // originally -5, now -10
      experience: 5
    },
    choiceB: {
      text: "Keep production manual.",
      money: 20,  // originally 10, now 20
      popularity: 10,  // originally 5, now 10
      operations: -10, // originally -5, now -10
      sustainability: 10,  // originally 5, now 10
      experience: 2
    }
  },

  // Scenario: eco_packaging (choices swapped, impacts doubled)
  {
    id: "eco_packaging",
    characterId: "greta",
    title: "Switch to Eco-Friendly Packaging",
    text: "Mr. Celebi, switching to biodegradable packaging is better for the planet but costly. Should we make the switch, or keep our current packaging?",
    choiceA: {
      text: "Keep current packaging.",
      money: 10,   // originally 5, now 10
      popularity: -10, // originally -5, now -10
      operations: 0,
      sustainability: -20, // originally -10, now -20
      experience: 2
    },
    choiceB: {
      text: "Switch to eco-friendly packaging.",
      money: -30, // originally -15, now -30
      popularity: 10,  // originally 5, now 10
      operations: 0,
      sustainability: 30, // originally 15, now 30
      experience: 3
    }
  },

  // Scenario: fair_trade (choices not swapped, impacts doubled)
  {
    id: "fair_trade",
    characterId: "greta",
    title: "Support Fair Trade Coffee Farmers",
    text: "Mr. Celebi, partnering with fair trade groups could improve our image but increase costs. Should we do it, or stick with our current sourcing?",
    choiceA: {
      text: "Partner with fair trade groups.",
      money: -20, // originally -10, now -20
      popularity: 20, // originally 10, now 20
      operations: 10,  // originally 5, now 10
      sustainability: 20, // originally 10, now 20
      experience: 3
    },
    choiceB: {
      text: "Stick with current sourcing.",
      money: 10,   // originally 5, now 10
      popularity: -10, // originally -5, now -10
      operations: -10, // originally -5, now -10
      sustainability: -10, // originally -5, now -10
      experience: 2
    }
  },

  // Scenario: employee_wages (choices swapped, impacts doubled)
  {
    id: "employee_wages",
    characterId: "bella",
    title: "Increase Employee Wages",
    text: "Mr. Celebi, our staff is asking for better wages. Should we give a big raise, or offer a small bonus to save money?",
    choiceA: {
      text: "Offer a small bonus.",
      money: -10,  // originally -5, now -10
      popularity: 10,  // originally 5, now 10
      operations: 0,
      sustainability: 0,
      experience: 2
    },
    choiceB: {
      text: "Give a big raise.",
      money: -40, // originally -20, now -40
      popularity: 30, // originally 15, now 30
      operations: 0,
      sustainability: 0,
      experience: 3
    }
  },

  // Scenario: training_programs (choices not swapped, impacts doubled)
  {
    id: "training_programs",
    characterId: "bella",
    title: "Implement Employee Training Programs",
    text: "Mr. Celebi, thorough training for all staff could improve service but is costly. Should we train everyone, or just new hires?",
    choiceA: {
      text: "Train all staff.",
      money: -30, // originally -15, now -30
      popularity: 20, // originally 10, now 20
      operations: 10,  // originally 5, now 10
      sustainability: 0,
      experience: 3
    },
    choiceB: {
      text: "Train only new hires.",
      money: -10,  // originally -5, now -10
      popularity: 4,  // originally 2, now 4
      operations: 4,  // originally 2, now 4
      sustainability: 0,
      experience: 2
    }
  },

  // Scenario: coffee_prices (choices swapped, impacts doubled)
  {
    id: "coffee_prices",
    characterId: "max",
    title: "Adjust Coffee Prices",
    text: "Mr. Celebi, raising prices could increase profits but upset customers. Should we raise them, or keep prices steady?",
    choiceA: {
      text: "Keep prices steady.",
      money: -10,  // originally -5, now -10
      popularity: 20, // originally 10, now 20
      operations: 0,
      sustainability: 0,
      experience: 2
    },
    choiceB: {
      text: "Raise prices.",
      money: 40,  // originally 20, now 40
      popularity: -30, // originally -15, now -30
      operations: 0,
      sustainability: 0,
      experience: 3
    }
  },

  // Scenario: new_technology (choices not swapped, impacts doubled)
  {
    id: "new_technology",
    characterId: "tom",
    title: "Invest in New Technology",
    text: "Mr. Celebi, upgrading our coffee machines could improve quality and stocks but is expensive. Should we upgrade, or stick with old equipment?",
    choiceA: {
      text: "Upgrade the machines.",
      money: -50, // originally -25, now -50
      popularity: 20, // originally 10, now 20
      operations: 30, // originally 15, now 30
      sustainability: 0,
      experience: 5
    },
    choiceB: {
      text: "Stick with old equipment.",
      money: 20,  // originally 10, now 20
      popularity: -10, // originally -5, now -10
      operations: -10, // originally -5, now -10
      sustainability: 0,
      experience: 2
    }
  },

  // Scenario: cut_costs (choices swapped, impacts doubled)
  {
    id: "cut_costs",
    characterId: "tom",
    title: "Cut Operational Costs",
    text: "Mr. Celebi, reducing staff hours could save money but hurt morale and service. Should we cut hours, or find other efficiencies?",
    choiceA: {
      text: "Find other efficiencies.",
      money: 10,   // originally 5, now 10
      popularity: -4, // originally -2, now -4
      operations: -4, // originally -2, now -4
      sustainability: 0,
      experience: 2
    },
    choiceB: {
      text: "Reduce staff hours.",
      money: 30,  // originally 15, now 30
      popularity: -20, // originally -10, now -20
      operations: -10, // originally -5, now -10
      sustainability: 0,
      experience: 3
    }
  },

  // Scenario: tax_breaks (choices not swapped, impacts doubled)
  {
    id: "tax_breaks",
    characterId: "greta",
    title: "Lobby for Tax Breaks",
    text: "Mr. Celebi, hiring lobbyists could secure tax breaks but is costly. Should we hire them, or accept current taxes?",
    choiceA: {
      text: "Hire lobbyists.",
      money: -40, // originally -20, now -40
      popularity: 0,
      operations: 0,
      sustainability: 30, // originally 15, now 30
      experience: 3
    },
    choiceB: {
      text: "Accept current taxes.",
      money: -10,  // originally -5, now -10
      popularity: 0,
      operations: 0,
      sustainability: -10, // originally -5, now -10
      experience: 2
    }
  },

  // Scenario: environmental_regulations (choices swapped, impacts doubled)
  {
    id: "environmental_regulations",
    characterId: "greta",
    title: "Comply with New Environmental Regulations",
    text: "Mr. Celebi, new regulations require us to reduce waste. Should we invest in compliance now, or delay and risk fines?",
    choiceA: {
      text: "Delay compliance.",
      money: 20,  // originally 10, now 20
      popularity: -10, // originally -5, now -10
      operations: 0,
      sustainability: -30, // originally -15, now -30
      experience: 2
    },
    choiceB: {
      text: "Invest in compliance.",
      money: -30, // originally -15, now -30
      popularity: 10,  // originally 5, now 10
      operations: 0,
      sustainability: 20, // originally 10, now 20
      experience: 3
    }
  },

  // Scenario: expand_country (choices not swapped, impacts doubled)
  {
    id: "expand_country",
    characterId: "max",
    title: "Expand into a New Country",
    text: "Mr. Celebi, entering a high-growth market could be lucrative but risky. Should we expand internationally, or grow domestically?",
    choiceA: {
      text: "Enter the new market.",
      money: -60, // originally -30, now -60
      popularity: 20, // originally 10, now 20
      operations: -20, // originally -10, now -20
      sustainability: 0,
      experience: 5
    },
    choiceB: {
      text: "Grow domestically.",
      money: -20, // originally -10, now -20
      popularity: 10,  // originally 5, now 10
      operations: -10, // originally -5, now -10
      sustainability: 0,
      experience: 2
    }
  },

  // Scenario: local_partnership (choices swapped, impacts doubled)
  {
    id: "local_partnership",
    characterId: "bella",
    title: "Partner with a Local Business",
    text: "Mr. Celebi, partnering with a local business could boost our image but share profits. Should we partner, or stay independent?",
    choiceA: {
      text: "Stay independent.",
      money: 10,   // originally 5, now 10
      popularity: -10, // originally -5, now -10
      operations: -10, // originally -5, now -10
      sustainability: 0,
      experience: 2
    },
    choiceB: {
      text: "Form a partnership.",
      money: -30, // originally -15, now -30
      popularity: 20, // originally 10, now 20
      operations: 10,  // originally 5, now 10
      sustainability: 0,
      experience: 3
    }
  },

  // Scenario: mobile_app (choices not swapped, impacts doubled)
  {
    id: "mobile_app",
    characterId: "max",
    title: "Launch a Mobile App for Ordering",
    text: "Mr. Celebi, a mobile app could make ordering easier but is costly. Should we build it, or stick to in-store orders?",
    choiceA: {
      text: "Build the app.",
      money: -40, // originally -20, now -40
      popularity: 30, // originally 15, now 30
      operations: 0,
      sustainability: 0,
      experience: 5
    },
    choiceB: {
      text: "Stick to in-store orders.",
      money: 20,  // originally 10, now 20
      popularity: -10, // originally -5, now -10
      operations: 0,
      sustainability: 0,
      experience: 2
    }
  },

  // Scenario: franchise (choices swapped, impacts doubled)
  {
    id: "franchise",
    characterId: "max",
    title: "Offer Franchise Opportunities",
    text: "Mr. Celebi, franchising could bring quick money but risk our brand. Should we allow it, or keep stores company-owned?",
    choiceA: {
      text: "Keep stores company-owned.",
      money: -20, // originally -10, now -20
      popularity: 10,  // originally 5, now 10
      operations: 0,
      sustainability: 0,
      experience: 2
    },
    choiceB: {
      text: "Enable franchising.",
      money: 50,  // originally 25, now 50
      popularity: -20, // originally -10, now -20
      operations: 0,
      sustainability: 0,
      experience: 5
    }
  },

  // Scenario: wellness_programs (choices not swapped, impacts doubled)
  {
    id: "wellness_programs",
    characterId: "bella",
    title: "Invest in Employee Wellness Programs",
    text: "Mr. Celebi, wellness programs could improve staff health but cost money. Should we offer full plans, or basic benefits?",
    choiceA: {
      text: "Offer full wellness plans.",
      money: -30, // originally -15, now -30
      popularity: 20, // originally 10, now 20
      operations: 0,
      sustainability: 0,
      experience: 3
    },
    choiceB: {
      text: "Offer basic benefits.",
      money: -10,  // originally -5, now -10
      popularity: 4,  // originally 2, now 4
      operations: 0,
      sustainability: 0,
      experience: 2
    }
  },

  // Scenario: subscription_service (choices swapped, impacts doubled)
  {
    id: "subscription_service",
    characterId: "tom",
    title: "Introduce a Subscription Service",
    text: "Mr. Celebi, a coffee subscription could provide steady income but strain logistics. Should we launch it, or focus on in-store sales?",
    choiceA: {
      text: "Focus on in-store sales.",
      money: 20,  // originally 10, now 20
      popularity: -10, // originally -5, now -10
      operations: 10,  // originally 5, now 10
      sustainability: 0,
      experience: 2
    },
    choiceB: {
      text: "Launch the subscription.",
      money: -40, // originally -20, now -40
      popularity: 20, // originally 10, now 20
      operations: -20, // originally -10, now -20
      sustainability: 0,
      experience: 5
    }
  },

  // Scenario: new_brewing_method (choices not swapped, impacts doubled)
  {
    id: "new_brewing_method",
    characterId: "bella",
    title: "Develop a New Coffee Brewing Method",
    text: "Mr. Celebi, innovating a unique brewing method could set us apart but requires investment. Should we innovate, or stick with traditional methods?",
    choiceA: {
      text: "Innovate a new method.",
      money: -50, // originally -25, now -50
      popularity: 30, // originally 15, now 30
      operations: 20, // originally 10, now 20
      sustainability: -10, // originally -5, now -10
      experience: 5
    },
    choiceB: {
      text: "Stick with traditional methods.",
      money: 20,  // originally 10, now 20
      popularity: -10, // originally -5, now -10
      operations: -10, // originally -5, now -10
      sustainability: 10,  // originally 5, now 10
      experience: 2
    }
  },

  // Add secondary character scenarios (unchanged position in code structure)
  ...secondaryScenarios
];